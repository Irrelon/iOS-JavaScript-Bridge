    ...
    : JSGlobalObject(globalData, structure)
{
    ...
    putDirectFunction(globalExec(), new (globalExec()) JSFunction(globalExec(), this, functionStructure(), 1, Identifier(globalExec(), "call_objc_funtion"), functionCallObjc));
...
EncodedJSValue JSC_HOST_CALL functionCallObjc(ExecState* exec)
{
	for (unsigned i = 0; i < exec->argumentCount(); ++i) {
		...
		printf("Call Objc: %s", exec->argument(i).toString(exec).utf8().data());
	}

	return JSValue::encode(jsString(exec, UString("return form functionCallObjc")));
}
...
int jscmain(int argc, char** argv, JSGlobalData* globalData)
{
	...
	{
		//printf("test");
		UString str("bludgeonArguments();");
		Completion completion = evaluate(globalObject->globalExec(), globalObject->globalScopeChain(), makeSource(str));
		printf("@@@:%s\n", completion.value().toString(globalObject->globalExec()).utf8().data());
	}
